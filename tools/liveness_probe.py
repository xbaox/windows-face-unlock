#!/usr/bin/env python3
"""liveness_probe.py -- Stage 2 probe: InsightFace 106-pt landmarks + blink (EAR).

Two modes:
--map  (default): explore landmarks. Draw the 106 points, mark the detector's two
                  eye-centers, label ONLY the points nearest each eye-center (to read off
                  eye indices), and show a live index-agnostic openness + blink counter.
                  'p' dumps eye-region indices/coords. 'q'/ESC quits.
--ear           : canonical EAR + blink using the SHARED module face_service.liveness
                  (same EYE_IDX / compute_ear / BlinkDetector that the service will use).

Reuses the Stage-1 CUDA fix (_prep_cuda_dlls) so it runs on the GPU. Read-only webcam
probe -- does not touch the service, recognizer, config, or enrolled data.

Run from repo root (C:\\dev\\windows-face-unlock):
    python tools\\liveness_probe.py            # map mode (default)
    python tools\\liveness_probe.py --ear      # canonical EAR via shared module
...
# Stage-1 GPU fix + provider selection (reused verbatim).
from face_service.recognizer import _prep_cuda_dlls, _select_providers
# Shared liveness primitives -- single source of truth for EAR/indices/blink.
from face_service.liveness import EYE_IDX, compute_ear, BlinkDetector, EAR_THRESH
...
def _fps(prev, smooth):
    now = time.time()
    return now, (0.9 * smooth + 0.1 * (1.0 / max(1e-3, now - prev)))
...
def run_map(app, cam_index):
    ...
    while True:
        ...
        if faces:
            ...
            if not hasattr(f, "landmark_2d_106") or f.landmark_2d_106 is None:
                ...
                for c in kps[:2].astype(int):
                    cv2.drawMarker(frame, tuple(c), (255, 255, 0), cv2.MARKER_CROSS, 14, 2)
                for i in eye_idx:
                    p = lmk[i].astype(int)
        ...
        cv2.putText(frame, info, (10, 26), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2, cv2.LINE_AA)
        ...
        if key == ord('p') and snapshot is not None:
            ...
            def dump(name, idx):
                print(f"\n[{name}] landmark indices nearest eye-center (index: x, y):")
                for i in idx:
                    print(f"  {int(i):3d}: {lmk[i][0]:6.1f}, {lmk[i][1]:6.1f}")

            dump("eye-A (kps[0])", li)
            dump("eye-B (kps[1])", ri)
            print("\nOrder for canonical EAR: [outer, top1, top2, inner, bottom2, bottom1]")
    cap.release()
    cv2.destroyAllWindows()


def run_ear(app, cam_index):
    """Canonical EAR + blink via the shared face_service.liveness module."""
    cap = open_cam(cam_index)
    det = BlinkDetector()   # same class the service will use; EYE_IDX/threshold from module
    print(f"[probe] EAR mode via face_service.liveness (thresh={EAR_THRESH}, "
          f"EYE_IDX={EYE_IDX}). 'q'/ESC quits.")
    while True:
        ok, frame = cap.read()
        if not ok:
            break
        ...
        if faces:
            f = largest(faces)
            lmk = f.landmark_2d_106.astype(np.float32)
            e = det.update(lmk)                         # drives the real state machine
            state = det.state.name
            for side in ("left", "right"):
                for i in EYE_IDX[side]:
                    cv2.circle(frame, tuple(lmk[i].astype(int)), 2, (0, 255, 0), -1)
            info = f"EAR={e:.3f} thr={EAR_THRESH} state={state} blinks={det.blinks}"
        cv2.putText(frame, info, (10, 26), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2, cv2.LINE_AA)
...
def main():
    ...
    ap.add_argument("--ear", action="store_true", help="canonical EAR via shared liveness module")
