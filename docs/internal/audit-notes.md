...
**Итог (Этап 8, D-37, окно 8c-10):** System Protection на C: оставлена включённой — решение архитектора по
делегированию Bao; точки восстановления #32 и #33 сохранены. Напоминание закрыто (раздел «Этап 8», 8c-10).
...
- **Death-wait вместо слепого sleep в ЧЕТЫРЁХ рестарт-путях:** `tools/watchdog.py:133-147` (`_wait_dead()`, вызов на `:156`, вместо `time.sleep(1.0)`), `tools/register_tasks.ps1:35-42` (вместо `Start-Sleep 1`), `tools/restart_service.ps1:25-32` и `tools/clean_restart.ps1:21-28` (вместо `Start-Sleep 2`). Ожидание **ограничено** (`_DEATH_WAIT_S = 10.0`, опрос `_DEATH_POLL_MS = 200`) и никогда не висит. ⚠️ **Точность:** подтверждающий лог `all killed processes confirmed gone` есть **в 1 из 4** путей (`watchdog.py:147`); три `.ps1` на успехе молчат и выводят только `Write-Warning` по таймауту. *(Этап 9, D-127: в `tools/watchdog.py` death-wait теперь `_kill_and_wait()` — `psutil.wait_procs` с тем же потолком 10 с, без PowerShell; номера строк выше относятся к версии Этапа 7.)*
...
### 7g — приёмка инсталлера (окно ротации)

> Блок приёмки: код заморожен, правятся только `.iss` и документация. Полный
> closeout-реестр 7f/7g — на мерже; здесь лежат только те факты, у которых есть
> адресат за пределами блока.

**A. Двойной трей: причина, фикс, остаток.** Интерактивная установка с галкой
«Launch …» на финишной странице подняла **второй** трей поверх уже работавшего.
Причина не в галке самой по себе, а в том, что шаг 2 из `[Run]` таски не только
регистрирует, но и **стартует** их: регистратор в ветке `-Action Register`
заканчивается проходом `Start-ScheduledTask`, а `FaceUnlock-Presence` — это и
есть трей-EXE. Галка удалена из `installer.iss` (`7de9892`); в `[Run]` больше
ничего не тронуто, `AppVersion` / `OutputBaseFilename` остались `0.1.0`. Корень —
у трея нет single-instance mutex, в отличие от сервиса, — **в 7g не чинится**
(код заморожен) и заведён как `KNOWN_ISSUES` №4 → Этап 8. Второй вектор
автозапуска искался и не нашёлся: трей-EXE упомянут в `.iss` ровно трижды
(`#define`, ярлык `[Icons]` в меню «Пуск», и удалённая строка), `{userstartup}` /
`{commonstartup}` в скрипте нет, ключа `Run` в `[Registry]` тоже — там только
`InstallLocation` и `Version`.

**B. Наблюдение оператора 19:51:04 (wedge-recovery камеры после force-kill) —
НЕ ЗАСЧИТАНО.** Строки нет ни в живых логах, ни в одной из сохранившихся копий,
поэтому наблюдение не заносится в реестр никак: ни четвёртым негативным прогоном
гипотезы `KNOWN_ISSUES` §2, ни фактом самовосстановления камеры. Сверка по
файлам, а не пересказ:

- **Живых строк нет.** `19:51:04` не встречается в `~\.face-unlock\*.log*` ни
  разу, а за всё окно `19:46:07`–`19:58:21` там **ноль строк** любого
  происхождения. `service.log` идёт встык: `14:40:06 FaceService stopped` →
  `19:59:10 FaceService starting`. Окно не пустое — оно отсутствующее.
- **Почему отсутствующее.** Логи окна писал стек, установленный в Program Files,
  и лежали они в его `~\.face-unlock`. Деинсталляция снесла каталог данных
  вместе с ними: `pp-uninstall-01.log:6` — `Uninstall command line: … /VERYSILENT
  /SUPPRESSMSGBOXES /NORESTART /REMOVEDATA …`.
- **Границы окна — по Inno-логам** (`C:\dev\d-series\`). Начало —
  `pp-install-01.log:10702`, `2026-08-08 19:46:07.828   Log closed.`; конец —
  `pp-uninstall-01.log:2385`, `2026-08-08 19:58:21.875   Log closed.`
- **Восстановление не помогает по датам.** Снапшот
  `C:\dev\b5-snapshots\data-after-install` — срез **≤ 14:53:03** (самый свежий
  файл в нём, `watchdog.log`; его `service.log` — ровно `14:40:06`, тот самый
  стык). Бэкап `C:\dev\face-unlock-backup-20260807-175801` — свежайший файл
  `2026-08-07 17:20:32`. Обе копии старше окна, поэтому его строк не содержат в
  принципе.
- **Как выглядела бы улика.** Форма записи про клин и выход из него в логах
  существует ровно одна — пара `face_service.camera_open`: «camera open wedged
  after 5.0s; abandoning attempt, not retrying» → «wedged camera open completed
  after N s; released abandoned capture». Она срабатывала пять раз (30.07,
  02.08 ×2, 03.08 ×2 — все в `service.log.1`) и **08.08 не встречается вовсе**.

`KNOWN_ISSUES` §2 остаётся как есть, включая «на сегодня — только ребутом»:
незасчитанное наблюдение не двигает ни статус записи, ни счёт негативных
прогонов — их по-прежнему три.

**Урок.** Архив прод-логов обязан идти ДО любого деструктивного шага, а
наблюдение без переживших строк в реестр не попадает — та же дисциплина, по
которой в 7c-J собственная аудит-запись была помечена опровергнутой: запись,
принятая без улик, сама по себе дефект.

**C. Наблюдаемость деинсталла: РЕШЕНИЕ архитектора, а не дефект.** stdout
регистратора в Inno-лог не попадает **by design** — Inno не захватывает stdout
дочернего процесса. Решено не захватывать. Авторитетным доказательством сноса
остаётся связка «семь нулей» живой приёмки + exit code регистратора в Inno-логе;
собственный файл-лог регистратора — кандидат Этапа 8, а не долг 7g.

**D. Сериализация setup/unins и прецедент 2026-08-08.** Setup и деинсталлятор
никогда не запускаются параллельно. Прецедент: `unins000 /REMOVEDATA` был
запущен поверх ещё шедшей тихой установки → подавленный msgbox ответился
авто-Abort → откат → **четыре шага приёмки аннулированы**.

Гонка видна прямо в таймстемпах Inno-логов (`C:\dev\d-series\`), если выложить
их по времени: деинсталлятор открылся через **23 секунды** после установщика и
успел закрыться раньше, чем тот дошёл до отката.

| файл:строка | строка |
|---|---|
| `pp-install-02-overlive.log:1` | `2026-08-08 19:57:50.547   Log opened. (Time zone: UTC-04:00)` |
| `pp-uninstall-01.log:1` | `2026-08-08 19:58:13.571   Log opened. (Time zone: UTC-04:00)` |
| `pp-uninstall-01.log:2385` | `2026-08-08 19:58:21.875   Log closed.` |
| `pp-install-02-overlive.log:10932` | `2026-08-08 19:59:24.290   Rolling back changes.` |
| `pp-install-02-overlive.log:12998` | `2026-08-08 19:59:24.949   Log closed.` |

Правило: следующий
запуск — только после `Log closed` предыдущего в его Inno-логе; молчание тихого
прогона до 15 минут — норма, а не признак зависания; `Ctrl+C` запрещён; длинные
ожидания — циклом с прогресс-эхом, чтобы «идёт» и «повисло» различались.
Записано языком файла в `installer/README.md`, раздел «Serialization rule
(hard)».

**E. Лицо на пере-гоне: слепой frozen-сервис (`KNOWN_ISSUES` §5).** Дискриминатор
разложен по четырём шагам, и каждый следующий снимал объяснение предыдущего:
локскрин 09.08 14:13:50–54 — пять `verify` подряд `matches=0/2 best=1.000`, то
есть лицо не найдено, а не «не похоже»; после аренды камеры визардом (14:20:11 →
14:20:30) следующий `verify` в 14:32:25 такой же — «залежавшаяся капча» отпала;
десять проб в **разлоченной** сессии 14:45:49–14:50:02 при сидящем пользователе —
все `absent`, причём сервис классифицирует каждый кадр как `hint=none=3` при
`frames_ok=3` и `luma_max` 89–144, то есть кадры идут, они светлые, и это не §1 и
не §2; и контраст — превью визарда на том же бандле, той же камере и той же сцене
работает (наблюдение оператора). **Вердикт:** отказ реальный и воспроизводимый,
виновник неизвестен, улик для выбора между гипотезами (путь кадра / провайдер
onnxruntime во frozen / формат кадра) нет ни одной; разделяющий опыт `verify` на
dev-стеке не поставлен, поэтому «frozen» — описание места отказа, а не причина.
**Решение архитектора:** четвёрка инсталлера продолжается с запаркованным лицом,
а §5 держится как блокер мержа в `master` и выключения SP на `C:` до окна
расследования (нужна новая наблюдаемость — сохранение `verify`-кадра под
конфиг-флагом, то есть код и отдельная санкция).

**F. Dev-контроль: разделяющий опыт поставлен, оговорка из E снята.** Первый
заход 17:21–17:25 сорвался и это видно в логе, а не додумано: пять строк
`skip: remote context (remote-tool:remoting_host.exe(connected))` подряд — проба
не выполнялась вовсе. Во frozen-окне таких строк нет ни одной (за всё 09.08 до
сноса — ноль), там все тики `src=camera`, то есть remote как объяснение frozen'а
снят. Строгий повтор на dev-стеке (`FaceService starting 17:19:32,833`,
`InsightFace ready 17:19:46,964`, `model warmup ok in 20.40s`) дал обе стороны.
**True negative**, оператор вне кадра, 18:37:17–18:41:29 — десять проб `absent`,
все кадры `d=1.000`, `luma_max` 109.06–124.40, то есть на пустом кадре dev ведёт
себя ровно как frozen. **True positive**, оператор в кадре, 18:44:29–18:49:31 —
шесть подряд `presence probe: state=present src=camera real=True
mode=recognition`; сервисной INFO под них нет по устройству логирования
(`present` пишется в `debug`, `service.py:1068-1069`), зато соседние `18:51:32` и
`18:53:34` печатают настоящие дистанции `d=0.439` / `d=0.358` — чего во
frozen-окне не было ни разу. Отдельный `verify`-прогон не понадобился: проба и
`unlock` делят детекционно-матчевое ядро (`service.py:1031` → `verify_frame` →
`recognizer.py:476` `analyze_frame`; `unlock` зовёт тот же `analyze_frame` на
`service.py:668`). **Вердикт:** frozen-раскладка — установленная разделяющая
переменная (та же машина, камера, галерея, свет; dev видит, frozen слеп), но
причина внутри бандла не выбрана: гипотезы 1–3 остаются, скоуп сужен до
`dist\WindowsFaceUnlock`. Попутный факт трассировки, важный для окна:
слепой путь детектит InsightFace'ом (`det_10g.onnx`), а рабочее превью визарда —
YuNet'ом (`face_detection_yunet_2023mar.onnx`), поэтому контраст «визард видит»
контролем на бандл **не является**. **Решение архитектора:** §5 остаётся блокером
мержа и SP-off; окно расследования — следующее.

**G. Зачёт четвёрки пере-гона.** Четыре прогона инсталлера прошли по правилу
сериализации, каждый со своим Inno-логом в `C:\dev\d-series\`.

| прогон | границы | итог |
|---|---|---|
| `pp-install-03.log` | `Log opened 14:01:53.302` → `Installation process succeeded 14:03:39.737` → `Log closed 14:04:07.213` | стек поднялся в `14:03:46,803`, здоровье №1 в норме |
| `pp-install-04-overlive.log` | `Log opened 16:35:45.761` → `Installation process succeeded 16:37:13.431` → `Log closed 16:37:18.613` | поверх живого стека, без отката; стек пересоздан — `FaceService starting 16:37:20,060`, свежие PID (вотчдог `26848` вместо `21760`) |
| чекпойнты | `cp1-post-face` — `14:14:18`; `cp2-pre-unins` — `17:01:49` | **урок сработал буквально:** cp2 снят за **9 секунд** до `Log opened` деинсталлятора (`17:01:58.325`) |
| `pp-uninstall-02.log` | `Log opened 17:01:58.325` (`/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /REMOVEDATA`) → `Log closed 17:02:12.552` | снос чистый, матрица девяти нулей |

Три находки того же захода:

- **`[UninstallRun]` исполнился ДВАЖДЫ** — не гонка, а недостающий `RunOnceId`.
  В логе две полные пары подряд: `17:01:58.359` регистратор `-Action Unregister`
  → `Process exit code: 0` в `17:02:04.011`, следом `regsvr32 /u` → `0`; затем
  **та же пара снова** — регистратор в `17:02:04.061` → `0` в `17:02:10.914`,
  `regsvr32 /u` → `0` в `17:02:10.947`. Два `[UninstallRun]`-набора записаны
  двумя установками (14:03 и 16:37) поверх одного `unins000`, и без `RunOnceId`
  каждый отрабатывает свой. Последствий нет — обе операции идемпотентны, все
  четыре кода возврата нулевые, — но это живая улика долга. Этап 8.
- **Повторный запуск unins-блока оператором по уже чистой машине гонки не
  создал.** `Start-Process` не стартовал ничего (пустой `FilePath` —
  деинсталлятора на диске уже не было), и в `pp-uninstall-02.log` ровно один
  `Log opened`. Правило сериализации не нарушено.
- **Инструментальная заметка.** Логи — UTF-8; читать `-Encoding utf8`, иначе
  cp1252-консоль покажет `U+2014` как `â€”`. Артефакт читателя, не продукта:
  строгий UTF-8-декод файла проходит, байты тире — `E2 80 94`.

### 7k — окно расследования слепого frozen (2026-08-16)

> Окно нашло и убило причину `KNOWN_ISSUES` §5: три мины упаковки, коммиты
> `b627766`, `2d2b55d`, `b7d9ab7`. Сама причина, её механизм и доказательство
> лежат в §5 и здесь не дублируются — ниже только процессные факты, у которых
> есть адресат за пределами окна. Улики: `C:\dev\d-series\7k\` —
> `7k7d-verdict.txt`, `presence-final1..3.txt`, пред-архивы логов.

**А. Дефект проба-канона: холостой прогон 7k-3.** Проба выполнялась командой
`status` вместо `presence`. `status` камеру не открывает и распознавания не
касается, то есть прогон не мог ни подтвердить, ни опровергнуть ничего — а был
засчитан как результат. **Правило:** команда пробы называется в задании явно и
цитируется в отчёте вместе с ответом; «пробу сделал» без процитированного JSON
результатом не является.

**Б. Дефект смок-критерия: 3/3 без учёта сцены.** Критерий приёмки звучал как
«три пробы из трёх», и в нём не было ни освещения, ни поведения анти-экрана.
Ровно в эту дыру провалился смок 20:21: три `uncertain` подряд при полностью
рабочем распознавании (§5), которые по букве критерия читались как провал
детекции. **Правило:** критерий смока распознавания включает условия сцены и
ожидаемый класс кадра, а не только счёт.

**В. Дефект 7k-7b: живые пробы не дублировались на диск.** Результаты
существовали только в переписке, сверить их постфактум было нечем. Введена
дисциплина verdict-файлов: каждая живая проба пишется в
`C:\dev\d-series\7k\<имя>.txt` вместе с ответом сервиса, и отчёт ссылается на
файл, а не на память. Первые по этому правилу — `7k7d-verdict.txt` и
`presence-final1..3.txt`.

**Г. READY-паттерн v2: гонка пайпа на первой пробе.** В 7k-3ter первая проба
после подъёма стека уходила в пайп раньше, чем сервис успевал его создать.
Паттерн дополнен: сначала опрос пайпа до успеха, затем пауза, и только потом
первая проба. Прежняя версия ждала ПРОЦЕСС — а поднятый процесс ещё не означает
принимающий канал.

**Д. Находка: PF-таски указывают на установленную копию, не на `dist`.**
Регистратор в Installed-режиме строит путь как `<InstallDir>\<InstalledExe>`
(`tools/register_tasks.ps1:229`), а `-InstallDir` инсталлер передаёт как `{app}`
(`installer/installer.iss:124`). `{#BuildRoot}` из `.iss` живёт только на этапе
`[Files]` и в рантайм не переносится, так что таск физически не может указывать
на `dist`. Следствие для протокола: **смок на `dist` валиден только при явном
запуске из `dist`** — иначе меряется установленная копия (`2E56F179`), а не
собранная. Расхождение ответов двух копий в этом окне объясняется именно этим.

**Е. Канал: ~7 вложений Bao → архитектор пришли пустыми.** Файлы доезжали
нулевыми, и потерю замечали не сразу — то есть канал молча терял именно улики.
**Правило:** результат едет короткой текстовой выжимкой в теле сообщения
(команда, ответ, две-три ключевые строки лога); вложение — только дубль, никогда
не единственный носитель.

**Ж. Новая способность: архитектор читает запушенный код с GitHub напрямую.**
`origin` сейчас на `e6292e0`, и всё до этой точки архитектор смотрит сам.
Непушенное — `b627766`, `2d2b55d`, `b7d9ab7` и диаг-ветка — доступно только через
CC. **Следствие:** в задании указывается, лежит ли предмет за `origin`; от этого
зависит, нужна ли цитата в отчёте или достаточно ссылки.

**З. `build.py` — это `installer\build.py`.** Спотык 7k-6: путь набирался от
корня репозитория, где файла нет. Записано, чтобы следующее окно не потратило на
это ещё один заход.

### 7l — финальная миля: онбординг, установка, приёмка (2026-09-23/24)

> Окно закрыло `KNOWN_ISSUES` §5 (там же — полный акт с цитатами) и довело
> Этап 7 до входа лицом с локскрина на установленной копии. Ниже — процессные
> факты, решения и долги. Улики: `C:\dev\d-series\7l\` — `7l-recon-cc.md`,
> `7l-A-report.md`, `7l-A2-report.md`, `7l-B-report.md`, `7l-C-report.md`,
> verdict-файлы и сегменты логов в `smoke\`, `b\`, `c\`.

**А. Поправки к `7l-starter.md` (по рекону `7l-recon-cc.md`, раздел H).**
- **H1.** Стартер: HEAD `b7d9ab7`, ahead 3, «push не делался». Факт: HEAD
  `275c4e0` == origin, push 2026-08-17 13:02:53 (reflog). Стартер описывал
  состояние до собственного коммита.
- **H2.** Стартер: `origin/stage7-packaging = e6292e0`. Факт: `275c4e0`;
  архитектор с GitHub видел всё, включая три мины.
- **H3.** Стартер: путь установки per-user, `%LOCALAPPDATA%\Programs\…`. Факт:
  `PrivilegesRequired=admin` (`installer.iss`) → `{autopf}` = `C:\Program
  Files\WindowsFaceUnlock`, Uninstall-ключ в HKLM, per-user каталога нет.
- **H4.** Стартер: `dist\_internal\objects\meanshape_68.pkl`. Факт: путь
  относительно `dist\WindowsFaceUnlock\`.
- **H10.** «PF-таски указывают на новую копию» по пути не проверяется: установка
  per-machine в тот же каталог, путь в тасках не меняется. Новизна копии
  проверяется по mtime exe и по наличию `_internal\objects\` в PF (так и
  сделано в приёмке).

**Б. Смена протокола (с 7l).** CC ведёт и репо, и рантайм (сборка, смоуки,
установка, приёмка, пробы, блокировки). Bao — только UAC, камера и локскрин;
шаги Bao помечаются в задании `[BAO]` и в консоли баннером с паузой. Ускоренный
режим: окно идёт автономно до конца или до STOP, архитектор проверяет по
отчётам и verdict-файлам. Необратимые шаги (установка, мерж, деструктив) — только
по явной санкции в задании окна. При красном ничего не удаляется и не
деинсталлируется: стек остаётся как есть, вердикт на диск, STOP.

**В. Код окна (3 коммита, `stage7-packaging`).**
- `2f95623` — CP регистрируется по умолчанию. `[Tasks] cp` без `Flags:
  unchecked`: вход лицом с локскрина — это DoD; провайдер аддитивен, плитки
  PIN/пароля не трогаются. **Нюанс:** `UsePreviousTasks=yes` переносит прежний
  выбор в апгрейд, а каждая установка до 7l записала `Deselected Tasks = [cp]` —
  на такой машине галочка остаётся снятой. Сценарная установка обязана нести
  `/MERGETASKS="cp"` (записано в `installer\README.md`, «Scripted install»).
  После установки 7l ключ показывает `Selected Tasks = [cp]`.
- `a126295` — онбординг на странице Finish: «Save / Update your Windows
  password for face sign-in» (`--set-password`, две записи со взаимоисключающими
  `Check: CredentialsSaved`) и «Set up face recognition now» (`--enroll`);
  `postinstall runasoriginaluser skipifsilent`, без `nowait`. Флаги идут через
  роутер `presence_monitor\__main__.py`, второй трей не стартует — запрет
  Launch-галочки (KNOWN_ISSUES §4) цел.
- `1bfc7bf` — мастер энролла ждёт сервис до 90 с (пинг по пайпу в потоке,
  результат в Tk через `after()`), статус «Connecting to the Face Unlock
  service…», по таймауту — прежнее поведение. `CAMERA_LEASE_S`/`LEASE_RENEW_S`
  не менялись. **В проде** (установка 2026-09-23): `service answered after
  0.0s`, `camera lease acquired after 0.4s`. Гонку frozen-смок 7l-A воспроизвёл
  и закрыл: `answered after 21.1s`, `lease acquired after 21.4s`.

**Г. Исключение i18n.** Ключ `enroll.status.connecting` добавлен **во все 12
локалей** по прямому указанию архитектора (1d), вопреки политике 7c в шапке
`face_service/i18n.py` («новые ключи — только `_EN`/`_RU`»). Текст политики не
правился → долг Этапа 8 (ниже). Пины `presence_guards_selftest`: 228 → 229.

**Д. Итог 7l-A2: камера в норме.** Зонд `tools.camera_cold_probe` при
остановленном сервисе, 8 циклов на бэкенд: open (construct + first_read) DSHOW —
медиана 1.48 с, максимум 2.55 с (холодный цикл); MSMF — 1.40 / 1.63 с; все 16
открытий успешны; чужого держателя нет (ConsentStore). База по логам (интервал
`model warmup ok` → `camera warmup`, ~155 стартов с июля): обычно 2.5–3.0 с,
2.0–4.1 с. **Фраза «открытие ~7 с» из отчёта 7l-A — ошибка подсчёта CC**
(считал от `InsightFace ready`, то есть вместе с прогревом модели; верный
интервал смока-1 — 3.69 с). Заклинивание открытия (`wedged after 5.0s` →
`camera warmup skipped: camera busy` → захват освобождён сам) — **2 на ~155
стартов** (2026-08-11 и смоук-2 7l), оба раза за необычно долгим прогревом
модели, то есть под нагрузкой. Потолок `camera_open_attempt_cap_s = 5.0` не
менялся, config.toml не трогался.

**Е. Хронология 7l-B (2026-09-23, вечер) — STOP на приёмке.**
- Страховка: точка восстановления **#32 «pre-7l-install»**, SP на C: ВКЛ.
- Установка: Setup ~23 мин стоял на страницах мастера (Bao отходил), копирование
  ~1.5 мин; `regsvr32` = 0, `register_tasks` = 0, `Log closed`. 25-минутное окно
  наблюдения истекло за 11 с до открытия мастера лица — наблюдение продолжено,
  не красное.
- Ошибки харнесса приёмки (скрипты CC, не продукт): (1) `Grab` отдавал одиночную
  строку, `$x[-1]` брал символ → ложное «InsightFace ready missing»; фикс —
  всегда массив. (2) разбор JSON срезал всё после `[N ms]`, а pipe_client иногда
  печатает `[N ms]` до JSON → настоящая серия 3/3 (20:12:57) засчитана как 1/3;
  фикс — брать блок `{…}`.
- STOP: две серии 0/3 `absent` (20:16–20:17). Лицо в каждом кадре,
  `d=0.38–0.59`, `real=False`. Причина (разбор архитектора): голова повёрнута
  мимо камеры, наушники поднимают дистанцию (известно с 7-i), вечерний свет даёт
  `real=False`. Продукт не виноват; стек оставлен как есть.

**Ж. Итоги 7l-C (2026-09-24, дневной свет) — GREEN.**
- Presence: первая же серия 3/3 `present=True real=True` (245 / 177 / 190 мс).
- Verify: `NEEDS_GESTURE`, `d=0.133`, `margin=0.187`, `real=true`, luma 140.8.
- Локскрин, лицо: `unlock needs-gesture d=0.1319 margin=0.1881 281 ms
  gesture=blink` → `unlock_gesture blink passed=True reason=granted
  distance_best=0.1237`; разблокировка ~5 с после лока.
- Локскрин, PIN: камера закрыта, вход по PIN, `granted` в окне нет.
- Диагностика галереи (офлайн, buffalo_l с `landmark_3d_68`, 45 кадров): кадры
  2026-09-23 — yaw 1–4°, но **pitch −16.5…−24°** против −9…−14° в июле (голова
  опущена/камера выше), между собой плотные (`d≈0.02` к своему центроиду), до
  июльского центроида 0.18–0.25; центроиды июль↔сентябрь — 0.182. Галерея не
  испорчена (всё внутри 0.32), но мастер принял наклонённые кадры со статусом
  «Good — hold still» → долг Этапа 8.

**З. Verify в paranoid — `NEEDS_GESTURE` по дизайну.** У Bao
`liveness_mode = "paranoid"` (боевой режим 7-i): пассивный `verify` на
узнанное лицо всегда отвечает `match:false verdict:NEEDS_GESTURE`, на локскрине
плитка просит жест. Критерий приёмки verify в этом режиме — `NEEDS_GESTURE` при
`d ≤ 0.32` и `real=true`, а не `match:true`.

**И. Канон инсталлера.** `installer_output\WindowsFaceUnlock-Setup-0.1.0.exe` —
SHA-256 **`0BE85822E0558963F45F9E789C9D79745F58D318B7669FBDC8D0A370DB9EA385`**,
**1 625 120 201 B**, собран 2026-09-23 19:07 из `1bfc7bf` (verify-гейт = 0,
смоуки dist 1–2 GREEN до ISCC). Прежний `717040B6` в архиве
`C:\dev\d-series\7l\pre-build\`. Установлен на этой машине 2026-09-23 20:07.

**К. Долги Этапа 8 (сводно: перенесены из `7l-starter.md` + добавлены в 7l).**
Из 7k:
- Guard на None-кадр в warmup/get-пути; `exc_info` в `recognizer.py` для релиза
  (INFO слеп к причинам отказов) — обсудить формат.
- Анти-экран: luma-aware порог `hf` / вечерний режим — замороженные константы не
  трогать без акта.
- Класс-гард сборки: grep вендорных site-packages на `getattr(sys, 'frozen'` /
  `_MEIPASS` (мины класса 3).
- Смок-протокол в доки (яркий свет, посадка, критерий).
- Канал вложений: репорт бага в поддержку Anthropic (файлы приходят пустыми).

Из 7l:
- `tools/camera_busy_selftest.py` нестабилен по таймингу: проверка
  «returns at the ceiling» (cap 0.05 с, порог < 0.25 с) один раз дала 0.33 с под
  нагрузкой; 10/10 перепрогонов зелёные — поднять запас.
- Текст политики i18n в `face_service/i18n.py`: привести в соответствие с
  исключением `enroll.status.connecting` (или откатить 10 локалей).
- Single-instance для `--enroll` (KNOWN_ISSUES §4): второй мастер сейчас ничем
  не запрещён.
- Заклинивание открытия камеры под нагрузкой (2 на ~155 стартов) — наблюдение;
  потолок 5 с не менять без данных.
- `C:\Users\zabao\.face-unlock\debug_frames\` — 40 файлов, 26.7 MB кадров лица
  из 7h/7j: удалить или оформить (биометрия лежит вне инвентаря).
- Коуч позы в мастере энролла: сейчас «Good — hold still» при опущенной голове.
  Порог — по данным 7l-C: принятые июльские кадры pitch −9…−14°, сентябрьские
  −16.5…−24°, yaw везде ≤ 9.4°; кандидат — предупреждение при pitch ниже ~−15°
  (на этой геометрии камеры) и |yaw| > ~15°. Опционально — pose-QC при build.

## Этап 8 — сквозной аудит (8a), пакет фиксов (8b), живая приёмка (8c) — ✅ DONE (2026-09-25)

> Улики: `C:\dev\d-series\8a\` (аудит: `8a-report.md`, `findings.md`, `debts.md`,
> `security-checklist.md`, `dod.md`, `proposals.md`, ZIP `4ABB25B8…AEDD85`),
> `C:\dev\d-series\8b\` и `8b2\` (фиксы, селфтесты, смоуки, отчёты) и `C:\dev\d-series\8c\` (живая
> приёмка: `8cN-report.md`, `8cN-verdict.txt`, `8cN-evidence.zip`). Номера F-NN и D-NN — из `findings.md` /
> `debts.md` 8a; D-40 и D-41 заведены в 8c. Финальные DoD и §7 — `docs/stage8/`.

### 8a — итоги аудита (2026-09-24, `c05a981`)

- Вердикт **GREEN (аудит полный)**; RED-условий не было. Шаги 6, 7, 10, 11 у CC
  остановил классификатор — их дописал архитектор по уликам.
- Счёт: 53 находки (P0 1 · P1 7 · P2 33 · P3 12) и 39 долгов. MASTER §7 **не
  пройден**, DoD **не выполнен** — всё незакрытое покрыто нарезкой 8b (код) и 8c
  (живая приёмка).
- P0: **F-01** — каталог данных открыт `BUILTIN\Users` на изменение (строку
  `Permissions: users-modify` ввёл `65e99ab`, 7d — регресс нашими руками), а сервис
  доверяет ему при старте. P1: F-02 (удаления без защиты от reparse), F-03
  (`WaitNamedPipeW` мог получить 0), F-04 (пароль не проверяется, CP глушит
  `ReportResult`), F-05 (`ok:false` = отсутствие), F-06 («Удалить энролл» не
  останавливает распознавание), F-07 (UAC другого админа), F-08 (сборка не гоняет
  frozen-гейт).
- Регресс-база: селфтесты 22/22, `camera_busy` ×20 макс. 0.06 с, парсер CP 28/28,
  `verify_frozen_entrypoints` против PF rc=0, прувер деинсталлятора PROOF HOLDS.
  Заморозки целы; процессные P3 — ниже, «Трассируемость».

#### АКТ 8b — санкции на правку замороженного (действуют только в окне 8b, поштучно)

**А-1. C++ CP.** Санкция 7-i исчерпана; эта — новая, только на перечисленное:
- F-03: `remaining()` считается один раз, в `WaitNamedPipeW` никогда не уходит 0, `Sleep` ограничен остатком;
- F-04, часть CP: в `ReportResult` при статусах «неверный пароль / отказ входа» — фиксированный текст на плитке («Сохранённый пароль не принят — войди по PIN и обнови пароль в Face Unlock») и запрет новых сканов до конца этой сессии LogonUI;
- F-24: учёт ссылок `DllCanUnloadNow`;
- F-25: затирание копий пароля в буферах `PipeClient` и в распарсенных строках;
- F-26: флаги SQOS identification при открытии пайпа;
- F-48, в объёме:
  - потолки длин (имя и домен ≤ 256, пароль ≤ 1024, без встроенных NUL) и защита от переполнения USHORT в `KerbPack`;
  - удержание C++-исключений на COM-границе и во входе потока;
  - удаление no-op `DisableThreadLibraryCalls(nullptr)` с комментарием про static CRT;
  - фиксированные тексты отказа по причинам (3–4 варианта) и кап длины подсказки с сервера ~120 символов без управляющих;
  - `ClearSecretsLocked` при сбое упаковки;
  - `DllRegisterServer` возвращает первую ошибку;
  - враждебные векторы и `IsHexToken` в тестах парсера;
- D-34: удалить устаревший комментарий в `guid.h:5`.

**Не трогать в CP:** GUID; регистрацию (ключи и значения); `kUnlockTimeoutMs 12000` / `kGestureTimeoutMs 15000`; async-дизайн (воркер, `CredentialsChanged`, selected-гейт); join-политику; вызов `SetFieldString` из воркера (F-48 / F11 — ПРИНЯТО, живьём работает, переделка рискованнее пользы); swallowed clicks (F14 — ПРИНЯТО).

**А-2. Периметр Этапа 4 и соседство.** Существующий порядок гейтов `unlock` / `unlock_gesture` (SYSTEM → lockout → токен → камера) сохраняется байт-в-байт. Разрешено только:
- F-20: `pipe_first_instance` добавить в храповик `_apply_posture_ratchet`;
- F-21: удалить команду `reset_lockout`, если в репо нет вызывающих; если есть — гейт SYSTEM + аудит;
- F-19: серверный дедлайн перед `_release_credentials` — если обработка запроса уже превысила бюджет CP за вычетом запаса, fail-closed без выдачи. Протокольные константы по образцу `GESTURE_TOKEN_TTL_S`: ~11 с для фазы 1, ~14 с для фазы 2. Аудит `granted` пишется только после успешной доставки ответа; недоставленный грант = отдельный outcome, lockout **не** сбрасывается;
- F-01 / P-03: добавочный fail-closed гейт **после** существующих. Если самолечение ACL каталога данных при старте не удалось, `unlock` / `unlock_gesture` отвечают отказом с новым reason `insecure-data-dir` (для CP это обычный провал → PIN);
- `_build_pipe_sa` и `FIRST_PIPE_INSTANCE` — байт-в-байт.

**А-3. Семантика liveness (F-11).** В жест-раунде с `identity=True` кадр с флагом анти-экрана обрабатывается как кадр с `is_match=False` (не доходит до `ch.feed`). Числа (`HF_THRESH`, `SCREEN_DOUBT_FRAC` и все прочие) не трогаются, новых порогов ноль.

**А-4. Семантика lockout (F-18, F-46).**
- Фаза 1 `unlock` не пишет страйк, если `frames_ok == 0`, галереи нет или моделей нет: это сбой устройства или движка, не лица. Ответ — честный reason.
- Во время аренды камеры энроллом `unlock` отвечает `camera-busy` (lockout-нейтрально).
- Числа 5 / 300 не трогаются. Правило фазы 2 из 7-i («не-ok движка в раунде → gesture-failed со страйком») не трогается.

**А-5. Новые НЕ-замороженные константы** (без новых полей Config):
- потолок `threshold` 0.5 в `validate` и GUI; сам 0.32 не меняется;
- клэмп `pause_camera` [5, 600] с;
- два UX-порога коуча позы в мастере: pitch ~−15°, |yaw| ~15°. Только предупреждение, приём кадров QC не меняется.

**А-6. Деструктив в 8b:**
- удалить `C:\Users\zabao\.face-unlock\debug_frames\` целиком (40 файлов, 26.7 MB): биометрию не архивировать и не копировать; до удаления — листинг (имена и размеры);
- диаг-ветки `diag/7j-debuglog` (`8f91510`) и `diag/7j2-debuglog` (`e260df5`) запушить тегами `archive/diag-7j-debuglog` и `archive/diag-7j2-debuglog`, после этого удалить локальные ветки;
- `7l-starter.md` перенести в `docs/handoffs/` (`git mv`).

**Жёсткий запрет:** не удалять и не переименовывать поля Config — живой `config.toml` Bao иначе падает в дефолты (unknown key → fast вместо paranoid).

Уточнение архитектора к А-6 (окно 8b): `debug_frames\` в 8b **не удалять** — их
удалит новый сервис при первом старте в 8c (purge-on-start), это и есть живая
проверка кода; в 8b только листинг (имена, размеры).

#### Решения по находкам и долгам (8a-report §4–5)

| Решение | Находки (F) | Долги (D) |
|---|---|---|
| **8b — код / доки** | F-01…F-13, F-15, F-16, F-18…F-26, F-29…F-46, F-48 (в объёме А-1), F-49, F-52 | D-07, D-09, D-10, D-12, D-13, D-14, D-16, D-17 (код), D-18, D-20, D-21, D-26, D-33, D-34, D-35, D-36 (§5), D-38, D-39 (= F-13) |
| **8b — notify-only (P-01)** | F-27, F-28 | — |
| **8c — живьём** | — | D-03, D-06 (санкция на временный `threshold = 0.26` + откат в том же прогоне), D-17 (живой мастер), D-29, D-31, D-36 (§7 после приёмки), D-37 (SP off — отдельной санкцией последним) |
| **ПРИНЯТО** | F-14 (same-user DoS, KI §7), F-17 (слепая камера = present, KI §6), F-47 (адаптация в paranoid — решение после DONE), F-50 (мёртвый тост `notify_enroll`, поле не удаляем), F-51 (похудение бандла — после DONE), F-53 (мёртвый код; жест-подсказки en/ru — решение 7-i) | D-02, D-04, D-05, D-08 (после DONE), D-15, D-19, D-22, D-23, D-30, D-32; D-24 — дефолт `auto_lock = true` остаётся, у Bao `false` — личная настройка |
| **ЗАКРЫТ / не-баг** | — | ЗАКРЫТ: D-01, D-25, D-27, D-28; не-баг: D-11 |

Политика i18n (D-13): новые ключи обязательно в `_EN` и `_RU`, прочие локали — по
желанию с фолбэком EN; исключение 7l (`enroll.status.connecting`) остаётся.

### Трассируемость (D-38, F-52)

**Шесть хэшей Этапа 3, которых нет в репо.** Ветка Этапа 3 была переписана
перед мержем; реальные коммиты — второй родитель мержа `16a0101`, соответствие
по заголовкам один к одному (8a `freeze-check.md` §0.2). Записи выше не
переписываются — читать через эту карту:

| В audit-notes (не существует) | Реальный коммит | Заголовок |
|---|---|---|
| `8c30224` | `fefb901` | feat(stage3): enrollment QC + adaptive gallery (Steps 0-2) + Stage-2 tail |
| `1dc496c` | `2e24e64` | feat(lowlight): Stage-3 Step 3 — low-light detection + too-dark gate |
| `c050f16` | `4e91d7a` | feat(lowlight): Stage-3 Step 3 — gated exposure boost + low_light_boost default |
| `25bbccc` | `c3bd546` | feat(camera): Stage-3 Step 4 — camera-busy detection |
| `d54a93d` | `a3506e7` | feat(service): Stage-3 Step 5 — graceful shutdown + watchdog supervisor |
| `259d30e` | `d8e65df` | chore: pin config.py/service.py to LF |

**Санкции задним числом (решение архитектора 8a, D-38).**
- `2f13b81` — статический CRT (`/MT`) для DLL Credential Provider: касание
  замороженного CP вне акта своего окна. 8a проверил: чистый дифф CP после
  `231ca39` — только `CMakeLists.txt` (+15), логика не тронута. **Санкционировано
  задним числом** этой записью. Следствие, закреплённое в 8b: при статическом CRT
  `DisableThreadLibraryCalls` не вызывать (комментарий в `dll.cpp`).
- `debug_dump_frames` (`e6292e0`, 7h) — новое поле Config (append-only, дефолт
  `False`), наблюдатель кадров для расследования §5. Значения существующих полей не
  менялись. **Санкционировано задним числом** этой записью; остаток — кадры в
  `debug_frames\` — закрывается purge-on-start (8b, F-12) с живой проверкой в 8c.

### 8b — пакет фиксов (2026-09-24, ветка `stage8-audit`)

Коммиты по пакетам (форма сообщений «дефект → последствие → фикс»; подробности —
`C:\dev\d-series\8b\8b-report.md`):

| Пакет | Коммит | Суть |
|---|---|---|
| A — каталог данных | `276f560` | самолечение ACL APP_DIR при старте, fail-closed `insecure-data-dir`, secrets перезаперты, purge `debug_frames`, reparse-safe удаления; installer без `[Dirs]` |
| B — пароль | `f2ff7e2` | одна проверка LogonUserW (network) перед сохранением; MSA / Entra ID — предупреждение, не блок |
| C — presence / мастер / трей | `49453a2` | `ok:false` не отсутствие; `clear_enrollment`; Заменить/Добавить; ошибка движка ≠ отсутствие; пауза вотчдога на build; single-instance; F-49 |
| D — сервис | `832e05d` | F-11 экран в жест-раунде; валидация / lockout.json / reload; `pause_camera` [5, 600]; F-18 без страйка; дедлайны 11 / 14 с и grant после доставки; `pipe_io` для всех клиентов; `reset_lockout` удалён; порог ≤ 0.5; F-42…F-46; кэш SID |
| E — вотчдог | `870ef4f` | pause-файл клэмп / isfinite / catch-all; guard итерации; kill по пути + сессии; граница клиента shutdown (F-37) |
| F — апдейтер | `4fa42b3` | notify-only (`APPLY_ENABLED = False`), 404 ≠ «сеть», пре-релизы игнорируются |
| G — инсталлятор | `a64bdab` | исходный юзер (ExecAsOriginalUser → `-UserSid`), `DisableDirPage`, лог регистратора + проверка кода, `RunOnceId`, PrepareToInstall = Stop, приоритет / рестарт тасков, xbaox, 0.1.1 |
| H — сборка / CI | `a6dadae` | build.py 0–7 в две половины, гейт со штампом, обязательная подпись DLL; класс-гард; пины CI; uninstall phase A без админа; sign_cp; register.ps1 |
| I — CP (А-1) | `75520b2` | F-03, F-04, F-24, F-25, F-26 (доказано на тестовом пайпе), F-48, D-34; парсер 101/101 |
| J — коуч позы | `0f6805b` | предупреждение по медиане позы сборки (pitch < −15°, \|yaw\| > 15°), приём QC не меняется |
| K — тест | `b932345` | запас тайминга `camera_busy_selftest` |
| L — доки | этот коммит | этот раздел, KNOWN_ISSUES §4 / §6–§8, MASTER §5, `docs/SMOKE.md`, дрейф F-52 |

**Отклонение J (записано в отчёте 8b).** Коуч позы предупреждает по итогам
...
### 8c — живая приёмка 0.1.1 (2026-09-25; репо `97b44d0`, сборка `1d5b8c0`)

Установлен `WindowsFaceUnlock-Setup-0.1.1.exe` (SHA-256 `667FEF0E…090D4F`) поверх 0.1.0. Конфиг Bao —
paranoid / 0.32 / `auto_lock = false` / en; SHA эталонного `config.toml` сверялся в каждом окне. Исполнял CC
(non-elevated; элевация — только установка 8c-1 по UAC Bao), физическое — Bao. Код в 8c не менялся. Улики —
`C:\dev\d-series\8c\` (`8cN-report.md`, `8cN-verdict.txt`, `8cN-evidence.zip`).

| Окно | Что | Итог |
|---|---|---|
| 8c-1 | Тихая установка (`/SILENT /MERGETASKS="cp"`), точка восстановления #33 | GREEN: exit 0; custody `aces_removed=124 relocked=2 problems=0` (F-01, F-23); purge `debug_frames\` — 40 файлов (F-12, D-16); исходный юзер по SID (F-07), регистратор OK (F-33); CP → PF, подписант `13F9BB62…649C` (F-13); Publisher xbaox |
| 8c-2 | presence ×3 + verify | GREEN: 3 / 3 present; verify `NEEDS_GESTURE`, d 0.158, `engine_errors` 0 (поле F-18) |
| 8c-3 | Локскрин: лицо; PIN при закрытой камере | GREEN: LogonUI (SYSTEM) прошёл SID-гейт при SQOS identification (F-26); фаза 1 — 188 мс → nod → granted, запись `granted` только после доставки (F-19); PIN пустил; страйк за светлую сцену без лица — по правилам |
| 8c-4 | Неверный сохранённый пароль (F-04) | GREEN: MSA-учётка — «сохранено, не подтверждено (1326)»; ровно 1 грант, новых сканов нет, текст плитки `kTextPasswordRejected`, PIN пустил; пароль возвращён из побайтного резерва → вход лицом |
| 8c-5 | Kill-смоук вотчдога (D-03, D-29) | GREEN: зависание → точечный kill по пути и сессии (F-36) и рестарт, ping ok через 83 с; падение → рестарт, ping ok через 86 с; custody идемпотентен |
| 8c-6 | fast-режим (D-06; DoD 7a / 7c) | GREEN по 7a: фаза 1 без жеста — 203–266 мс. 7c не собран: первый грант фазы 1 добавил адаптивный снимок (`store=1/10`), d упала до ~0.04, запас остался ≥ 0.10 |
| 8c-7 | Живой 7c + single-instance (F-31) | GREEN: fast / 0.15 / `adaptive_gallery = false` → фаза 1 d 0.0629, запас 0.0871 < 0.10 → `needs-gesture` → blink → granted; конфиг возвращён, SHA = эталон. Второй трей и второй мастер вышли с кодом 0 за 0.35 с; первый мастер закрыт штатно, аренда камеры снята; галерея не изменилась |
| 8c-8 | Ребут через Restart (D-31) | GREEN с оговоркой: стек по логону (+2.6 с), custody `aces_removed=0 relocked=0 problems=0`, 3 входа лицом; попытка Bao с фото — NOT_LIVE (экран 4/5 и 2/5). Restart дал автологон Windows (ARSO) с блокировкой — экрана до логона не было |
| 8c-8b | Холодный старт: Shut down → включение (D-31) | GREEN: без ARSO (power off, тип загрузки 0x1, ручной вход через 59 с, без автоблокировки); плитка до логона — «Face Unlock service unavailable. Use PIN or password.» (бюджет пайпа CP 12 с, F-03), PIN пустил; вход лицом — фаза 1 218 мс |

#### Решения архитектора 8c

- F-17 (слепая камера = present) — ПРИНЯТО как fail-safe (KNOWN_ISSUES §6); F-14 — ПРИНЯТО (§7); апдейтер
  notify-only до подписанных релизов (§8); тегов `v*` и GitHub-релизов не публиковать.
- F-37: граница graceful-клиента — 10 с (замер 1.0–1.13 с на прогретой машине; первый запуск после установки
  идёт под AV-сканом).
- F-18, нейтральная ветка («нет кадров», «темно») — ПРИНЯТО по селфтестам: живьём пришлось бы отключать камеру.
- D-17 (коуч позы) — ПРИНЯТО по селфтесту (литералы gallery-diag); живьём — при естественном переэнролле:
  полный переэнролл ради проверки перетряхнул бы рабочую галерею.
- Адаптивный снимок, который продукт добавил в 8c-6, оставлен — это штатная работа продукта.
- Рецепт 7c (8c-7) вместо временного порога 0.26:
  1. При `adaptive_gallery = true` `validate()` отвергает `threshold ≤ adaptive_margin` (0.17): `reload_config`
     вернул бы `invalid-config`, и прогон тихо остался бы в paranoid. На время прогона — `adaptive_gallery =
     false`: `_maybe_adapt_gallery` выходит сразу, адаптации нет ни при каком пороге.
  2. `reload_config` галерею не перечитывает: адаптивный снимок остаётся в матчинге. Поэтому порог считается от
     свежего `verify` в эталонном конфиге (`verify` не выдаёт грант, не адаптирует, lockout не трогает).
  3. Раунд жеста подаёт задаче только кадры с d ≤ порога (identity binding), поэтому d мерились и в позах
     жеста; порог — по формуле из замера поз (`8c7-T.txt`).

  Резерв `config.toml` лежит в папке улик, вне каталога данных, — удалять нечего. Правка — побайтно на месте,
  три строки, SDDL не меняется; возврат обязателен при любом исходе.
- Второй экземпляр трея и мастера выходит молча, без строки в логах, — так задумано в F-31: выход происходит до
  настройки логирования. Строка «already running» остаётся у запуска мастера из меню трея (`tray.py`).
- Служебный `8c7-restore.tmp` оставлен уликой: фильтр оболочки отбил команду с его удалением, обхода не было.
- D-31: Restart на этой машине даёт автологон с блокировкой (ARSO), экрана до логона нет. Поэтому сценарий
  «сервиса ещё нет» проверен отдельным холодным стартом (8c-8b); тумблер ARSO не трогали.
- Мерж `stage8-audit` → `master` — fast-forward (как в 7l, D-32).

#### Статус находок и долгов после 8c

- Живьём подтверждены: F-01, F-07, F-12, F-13, F-23, F-33 (8c-1); F-19, F-26 (8c-3); F-04 (8c-4); F-36 (8c-5);
  F-31 (8c-7); F-03 (8c-8b). F-11 — селфтесты 8b; живая попытка с фото отбита фазой 1 (8c-8).
- Долги: D-03 и D-29 ✅ 8c-5; D-06 ✅ 8c-7; D-31 ✅ 8c-8 / 8c-8b; D-36 ✅ этот коммит (чекбоксы §7 MASTER);
  D-17 — ПРИНЯТО (селфтест); D-37 — ПРИНЯТО в 8c-10 (ниже).
- **D-40 (P3), новый.** Сервис, стартовавший с `adaptive_gallery = false`, не грузит адаптивное кольцо
  (`recognizer.load`), а `reload_config` с `true` его не подгружает: матчинг идёт без сохранённых снимков, а
  первая адаптация перезапишет `adaptive.npz` одним снимком. Безопасность не затронута. ПРИНЯТО, после DONE.
- **D-41 (P3), новый.** Identity binding против низкого порога: на этой камере d поворота 0.229–0.250, кивка —
  0.138 (замер 8c-7). При пороге ниже ~0.27 жесты поворота не засчитываются. ПРИНЯТО — KNOWN_ISSUES §9.

#### Грабли 8c

- Любой грант фазы 1 в fast добавляет адаптивный снимок и опускает d: тесты тонкого запаса — до гранта и от
  свежего замера d.
- Пути продукта берутся из `FACE_UNLOCK_HOME`: PF-экзешник, запущенный из шелла с этой переменной, уйдёт в
  scratch. Переменную задавать только дочернему `pipe_client`.
- Ребут-окна: Restart → ARSO (автологон и блокировка); для экрана до логона — Shut down и включение. При Fast
  Startup `LastBootUpTime` на Shut down не меняется — холодный старт доказывается через User32 1074 (power off
  или restart), Kernel-Boot 27 (тип загрузки), новую сессию и тайминг аутентификации в Winlogon/Operational.
- Фильтр оболочки ложно срабатывает на команду, где рядом литерал `C:\Program Files\…` и `Remove-Item`. Не
  обходить: проверки повторять только на чтение, служебный файл оставлять.
- PS 5.1: `Start-Process -PassThru` отдаёт `ExitCode`, только если сразу после старта обратиться к `.Handle`.

#### Итог Этапа 8

Инварианты и заморозки целы: 8a `freeze-check`; в 8b правки замороженного — только по актам А-1…А-6; в 8c код
не менялся. DoD выполнен — `docs/stage8/dod.md`. Security-чеклист MASTER §7 пройден —
`docs/stage8/security-checklist.md`, чекбоксы §7 заполнены. KNOWN_ISSUES §4 закрыт, §9 заведён. D-37 закрыт
решением в 8c-10 (ниже). Документация Этапа 8 — master `88ddb12` (FF, 2026-09-25).
**Этап 8 — DONE. Проект — DONE (2026-09-25).**

#### 8c-10 — D-37: System Protection (2026-09-25)

Bao делегировал выбор архитектору с приоритетом качества. **Решение: System Protection на C: остаётся
включённой, точки восстановления #32 «pre-7l-install» и #33 «pre-8c-install» сохраняются. D-37 — ПРИНЯТО.**

Почему не выключаем:
- Сетка безопасности проекта больше не нужна: приёмка 8c пройдена, PIN пускает во всех проверенных сценариях
  (закрытая камера, неверный сохранённый пароль, экран до логона). Но у SP есть ценность и вне проекта — откат
  после неудачного обновления Windows или драйвера и после будущих версий нашего CP, который стоит на пути
  входа в систему.
- Асимметрия необратимости: выключение SP навсегда удаляет все точки, а включённую SP можно выключить в любой
  момент одним действием. Конкретной выгоды от выключения нет — место под точки ограничено квотой, старые точки
  Windows удаляет сама.
- Удалять только #32 и #33 при включённой SP тоже незачем: #33 — ближайшая точка отката до установки 0.1.1.
- По правилам проекта необратимый шаг требует явной санкции именно на это действие; делегирование выбора ею не
  является. Решение без необратимых шагов санкции не требует.

Если Bao позже захочет выключить SP — это обычная настройка Windows («Защита системы» → «Настроить» →
«Отключить защиту системы»), к проекту она больше отношения не имеет.

Снимок состояния SP (только чтение, элевация по UAC Bao) — `C:\dev\d-series\8c\8c10-sp.txt`.

---

## Этап 9 — аудит 9a, акт 9b, пакеты 9c (ветка `stage9`) — 9c DONE (2026-09-26), дальше 9e–9i

Цель этапа: другой человек скачал с GitHub, поставил, настроил лицо — и всё работает; публикация отдельным
репо. Источники решений: `docs/internal/act-9b.md` (копия акта, SHA-256 A46D8DA6…3FEF) > реестр 9a
(331 находка) > design-facts 9a1–9a7.

### 9a — построчный аудит (окна 9a-0…9a-7)
- Реестр 331 находка: P1 13, P2 60, P3 259; у каждой — решение R1–R20 и пакет 9c.
- Хотфиксы на master: `be70a4d` (B2b-06: `disable_tauri.ps1` снимал регистрацию Windows Hello Face),
  `3ca280d` (README: честная формулировка про фото/экран/видео).
- Замер подделки «до» (9a-5, 0.1.1): 0 проходов из 27; но в фазе 2 видео давало 108–114 кадров «как владелец»
  без флага экрана, а раунд моргания ни разу не выпал — отсюда R4.

### 9b — акт решений R1–R20
Строгий однопользовательский режим (R1), периметр пайпа без конфиг-ключей (R2), честная плитка с картой причин
и протоколом v2 (R3, §2.1), два движения головы + неподвижность + экранный слой (R4), страйк только за
настоящий провал (R5), энролл одного человека и калибровка поворота (R6), модели по согласию (R7, 9-02),
Unicode-безопасный cv2 (R8), custody fail-closed и поключевая деградация конфига (R9), камера по требованию
и состояние `unknown` (R10), watchdog с состоянием (R11), UI в одном Tk-потоке (R12), пароль с проверкой
входом (R13), WinRT-тосты (R14), апдейтер только проверяет (R15), EN/RU (R16), установка через exe продукта,
варианты CPU/GPU (R17), хэш-локи, порядок подписи, CI (R18), dev-раскладка только в админском дереве (R19),
инструменты и тесты (R20). Решения «решение нужно»: D-43 принято (в SECURITY), F-123 печать — известное
ограничение (в SECURITY), F-171 экранные дикторы — ограничение Tk (в доках).

### 9c — автономный прогон, коммиты в `stage9`
| пакет | коммит | находки | суть |
|---|---|---|---|
| 9c-1 | `9cec974`, `8cc2dc0`, `ce59ebf` | 55 fixed | периметр, плитка, протокол v2, владелец R1 |
| 9c-2 | `0834a4e` | 53 fixed | движок: два движения, честные страйки, один человек, пины моделей, поключевой конфиг |
| 9c-3 | `0657eaf` | 29 fixed | камера по требованию, имя камеры, ограниченное чтение, `unknown`, watchdog R11 |
| 9c-4 | `e6e3dfe` | 67 fixed | один Tk-поток, Settings, мастер, диалог пароля, тосты, апдейтер, EN/RU |
| 9c-5 | `00c67d2` | 78 fixed | задачи через exe, модели по согласию, варианты CPU/GPU, хэш-локи, CI |
| 9c-6 | (коммит пакета) | 49 | R20: `tools/diag/`, удаление 7 утилит, жёсткая изоляция селфтестов (`tools/testhome.py`, мета-тест `isolation_selftest`), фикстуры гейта (`verifier_selftest`), гигиена селфтестов D-136…D-148; новые английские доки README / INSTALL / SECURITY (+приватность) / THIRD_PARTY_NOTICES / CONTRIBUTING; тексты лицензий в сборке (`installer/notices.py`, F-260); внутренние документы → `docs/internal/` |

- Заморозки: `verify_frame` 50fd349d96c8 и `_prep_cuda_dlls` 1982e2cd40ca — MATCH на каждом пакете.
- Отложено на VM/живое (deferred: 9e): формы имени Entra/MSA (F-78/F-84), видимый тост (F-197), тёплая камера
  на блокировке, поведение буста на других камерах; подпись `.cat` для неподписанных DLL NVIDIA (F-263, 9f),
  замер времени CI (D-116, 9f), публичная подпись (F-70, 9f).
- Живые тесты, установка на ПК-1, изменение прода в 9c не выполнялись (правило окна).

### Решения 9c-6 (фиксирую)
- **Утилиты (R20, B13):** 8 диагностик → `tools/diag/` (пути к корню `parents[2]`; `shutdown_integration`:
  невзлетевший сервис — FAIL, «не может работать» — код 2, без слепого `shutdown` в общий пайп). Удалены
  `liveness_probe`, `pose_probe`, `onnx_probe`, `ort_cuda_check`, `pipe_test`, `recog_smoke`,
  `disable_tauri.ps1`. **`tools/enroll.py` удалён** (мастер заменяет его; F-255/F-256/D-132 закрыты удалением).
  **`fast_install.ps1` и `fetch_wheels.ps1` удалены** (их заменил `setup.ps1` с хэш-локами; D-131).
  **`tools/set_password.py` оставлен** для скриптов: пустой пароль — отказ, проверка входом Windows до
  сохранения (тот же путь, что диалог), коды выхода.
- **Изоляция селфтестов:** каждый тест до импорта продукта вызывает `testhome.isolate()` / `own_root()`;
  дом вне TEMP или равный/содержащий/внутри реального `%USERPROFILE%\.face-unlock` — отказ с кодом 2; поздняя
  изоляция (конфиг уже импортирован) — отказ; свой временный дом удаляется при выходе. Пропуск секции — FAIL без
  `--allow-skip`. Монкипатчи восстанавливаются после каждой тест-функции.
- **Доки:** пользовательские доки только английские и без ссылок во внутренние; `docs/internal/` в новый
  репо (9g) не переносится. LICENSE: строка апстрима сохранена, добавлена `Copyright (c) 2026 xbaox`.
